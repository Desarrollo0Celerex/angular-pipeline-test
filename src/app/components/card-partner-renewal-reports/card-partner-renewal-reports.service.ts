...
export class CardPartnerRenewalReportsService {
    ...
    downloadReport(partnerId: number, rangeStart: string, rangeEnd: string, formatType: number): Promise<void> {
        ...
        switch(this.selectedReportType) {
            case REPORT_TYPES.APPLIED_RENEWALS:
                return this._downloadReportAppliedRenewals(partnerId, filters, rangeField, rangeStart, rangeEnd, formatType, sortBy);

            default:
                return this._downloadReportPendingRenewals(partnerId, filters, rangeField, rangeStart, rangeEnd, formatType, sortBy);
        }
    }
    ...
    private _downloadReportAppliedRenewals(partnerId: number, filters: string, rangeField: string, rangeStart: string, rangeEnd: string, formatType: number, sortBy: string): Promise<void> {
        return new Promise((resolve) => {
            this._policyService.downloadReportPartnerAppliedRenewals(partnerId, filters, rangeField, rangeStart, rangeEnd, formatType, sortBy).then((response: any) => {
    ...
    private _downloadReportPendingRenewals(partnerId: number, filters: string, rangeField: string, rangeStart: string, rangeEnd: string, formatType: number, sortBy: string): Promise<void> {
        return new Promise((resolve) => {
            this._policyService.downloadReportPartnerPendingRenewals(partnerId, filters, rangeField, rangeStart, rangeEnd, formatType, sortBy).then((response: any) => {
              const filename = response.headers.get('content-disposition').split(';')[1].split('filename')[1].split('=')[1].split('"')[1].trim();
              const blob = new Blob([response.body], {type: response.type.toString()});
                  saveAs(blob, filename);
                  resolve();
