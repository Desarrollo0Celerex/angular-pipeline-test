...
export class CardPartnerPaymentReportsService {
    ...
    downloadReport(partnerId: number, rangeStart: string, rangeEnd: string, formatType: number): Promise<void> {
        switch(this.selectedReportType) {
            case REPORT_TYPES.APPLIED_PAYMENTS:
                return this._downloadReportAppliedPayments(partnerId, rangeStart, rangeEnd, formatType);

            default:
                return this._downloadReportPendingPayments(partnerId, rangeStart, rangeEnd, formatType);
        }
    }
    ...
    private _downloadReportAppliedPayments(partnerId: number, rangeStart: string, rangeEnd: string, formatType: number): Promise<void> {
        return new Promise((resolve) => {
            const rangeField: string = 'paymentDate';
            const sortBy: string = 'applicationDate';
            this._receiptPaidService.downloadReportPartnerAppliedPayments(partnerId, rangeField, rangeStart, rangeEnd, formatType, sortBy).then((response: any) => {
    ...
    private _downloadReportPendingPayments(partnerId: number, rangeStart: string, rangeEnd: string, formatType: number): Promise<void> {
        return new Promise((resolve) => {
            ...
            this._paymentService.downloadReportPartnerPendingPayments(partnerId, filters, rangeField, rangeStart, rangeEnd, formatType, sortBy).then((response: any) => {
              const filename = response.headers.get('content-disposition').split(';')[1].split('filename')[1].split('=')[1].split('"')[1].trim();
              const blob = new Blob([response.body], {type: response.type.toString()});
                  saveAs(blob, filename);
                  resolve();
