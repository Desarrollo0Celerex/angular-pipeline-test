...
export class ModalUpdateSinisterCertificateComponent implements OnChanges {
    ...
    updatePolicySinisterCertificate(): void {
        ...
        if(this.model.form.valid && !!this.sinisterData) {
            ...
            this.model.updatePolicySinisterCertificate(this.sinisterData).subscribe((policyInsuredId: string) => {
                ModalPlugin.hide(this.modalId);
                setTimeout(() => {
                    this._loadingService.hide();
                    AlertHelper.sinisterCertificateUpdated(this._reloadPage, this);
                }, 500);
            },
            (error: HttpError) => {
                switch(error.error) {
                    case ERROR_CODES.sinisterCertificateNotFound:
                        this.model.f.certificate.setErrors({sinisterCertificateNotFound: true});
                    break;
                }
                this._loadingService.hide();
            });
    ...
    private _reloadPage(context: ModalUpdateSinisterCertificateComponent): void {
