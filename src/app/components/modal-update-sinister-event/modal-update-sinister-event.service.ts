...
import { EMAIL_LENGTH, FILE_NAME_LENGTH, LONG_ALPHANUMERIC_LENGTH, MULTITEXT_LENGTH, OWN_NAME_LENGTH, SINISTER_EVENT_TYPES, INSURANCE_GROUPS } from '@constants/global';
...
export class ModalUpdateSinisterEventService {
    ...
    getSinisterEvent(sinisterEventData: SinisterEventDataSend): Observable<SinisterEvent> {
        const fields: string = 'sinisterEventTypeId,evidenceName,providerName,providerDate,valuationDate,authorizationDate,insuredNoticeDate,insuredAuthorizationDate,estimatedDeliveryDate,repairDate,deliveryDate,readmissionDate,providerFolio,providerBill,providerPhoneCodeId,providerPhoneNumber,providerEmail,observations,canNotifyInsured,sinisterResolutionId,currencyId,paymentMethodId,insuranceGroupId,sumInsuredChassis,deductibleChassis,sumInsuredAdaptation,deductibleAdaptation';
                ...
                switch (sinisterEvent.insuranceGroupId) {
                    case INSURANCE_GROUPS.VEHICLES:
                        this.form = this._formBuilder.group({
                            ...
                            evidenceName: [sinisterEvent.evidenceName, [Validators.minLength(FILE_NAME_LENGTH.MIN), Validators.maxLength(FILE_NAME_LENGTH.MAX), ValidatorsHelper.fileName]],
                            evidenceFile: [''],
                            sinisterResolutionId: [sinisterEvent.sinisterResolutionId, [Validators.required]],
                            providerDate: [sinisterEvent.providerDate, [Validators.required, ValidatorsHelper.date]],
                            valuationDate: [sinisterEvent.valuationDate, [Validators.required, ValidatorsHelper.date]],
                            sumInsuredChassis: [sinisterEvent.sumInsuredChassis, [Validators.required, ValidatorsHelper.amount]],
                            deductibleChassis: [sinisterEvent.deductibleChassis, [Validators.required, ValidatorsHelper.amount]],
                            sumInsuredAdaptation: [sinisterEvent.sumInsuredAdaptation, [Validators.required, ValidatorsHelper.amount]],
                            deductibleAdaptation: [sinisterEvent.deductibleAdaptation, [Validators.required, ValidatorsHelper.amount]],
                            providerBill: [sinisterEvent.providerBill, [ValidatorsHelper.amount]],
                            currencyId: [sinisterEvent.currencyId, [Validators.required]],
                            paymentMethodId: [sinisterEvent.paymentMethodId, [Validators.required]],
                            observations: [sinisterEvent.observations, [Validators.required, Validators.minLength(MULTITEXT_LENGTH.MIN), Validators.maxLength(MULTITEXT_LENGTH.MAX), ValidatorsHelper.multitext]]
                        });
                        break;
