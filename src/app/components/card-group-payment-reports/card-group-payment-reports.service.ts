...
export class CardGroupPaymentReportsService {
    ...
    private _generatePaymentRequests(parnerId: number, rangeField: string, rangeStart: string, rangeEnd: string): Observable<number[]> {
        ...
        const requestTotalGroupAppliedPayments: Observable<number> = this._receiptPaidService.getTotalGroupReceiptsPaid(parnerId, rangeField, rangeStart, rangeEnd);
        const requestTotalGroupPendingPayments: Observable<number> = this._paymentService.getTotalGroupPayments(parnerId, filters, rangeField, rangeStart, rangeEnd);
