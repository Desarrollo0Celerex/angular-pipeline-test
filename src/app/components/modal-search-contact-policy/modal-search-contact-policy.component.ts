...
export class ModalSearchContactPolicyComponent {
    ...
    getErrorMessage(constrolName: string): string {
        const control: AbstractControl | null = this.modalSearchContactPolicyService.searchForm.get(constrolName);
    ...
    onSubmitSearchPolicy(): void {
        ...
        if(this.modalSearchContactPolicyService.searchForm.valid) {
            ...
            this.modalSearchContactPolicyService.searchContactPolicy(this.contactId).subscribe( (res: HttpResponse) => {
                const totalFoundPolicies: number = res.data.items.length;
                // If there are no policies
                if(totalFoundPolicies === 0) {
                    this.isNoResults = true;
                    this._resetSearchForm(this.modalSearchContactPolicyService.f.policyNumber.value);
                } else {
                    this.isNoResults = false;
                    ModalPlugin.hide(this.modalId);
                    this._resetSearchForm();
