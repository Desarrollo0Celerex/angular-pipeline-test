...
const routes: any = {
    ...
    reportOpenSinisters: (workspaceId: string) => environment.apiUrl + '/workspaces/' + workspaceId + '/sinisters/reports/open',
...
export class SinisterService {
    ...
    downloadReportOpenSinisters(rangeField: string = '', rangeStart: string = '', rangeEnd: string = '', sortBy: string = '-createdAt', specialFilter: string = '', formatType: number) {
     ...
     */
    getPartnerSinisters(partnerId: string, page: number = 1, fields: string = '', filters: number[] = [], query: string = ''): Observable<HttpResponse> {
        const route: string = routes.partnerSinisters(this._workspaceId, partnerId);
        let params: HttpParams = new HttpParams();
        params = params.append('page', page.toString());
        if(!!fields) params = params.append('fields', fields);
        if(filters.length > 0) params = params.append('filter', this._getFilter(filters));
