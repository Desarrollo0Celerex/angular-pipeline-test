...
export class BusinessIntelligenceService {
    ...
    getContactAppliedRenewalsRate(contactId: string, rangeStart: string, rangeEnd: string): Observable<number> {
    ...
    getContactWalletDecreaseRate(contactId: string): Observable<number> {
        const route: string = routes.contactWalletDecrease(this._workspaceId, contactId);
        return this._httpClient.get<HttpResponse>(route).pipe(
            map((res: HttpResponse) => {
                return res.data;
            })
    ...
    getGroupAppliedPaymentsRate(groupId: string, rangeStart: string, rangeEnd: string): Observable<number> {
        const route: string = routes.groupAppliedPayments(this._workspaceId, groupId);
        let params: HttpParams = new HttpParams();
        if(!!rangeStart) params = params.append('rangeStart', rangeStart);
        if(!!rangeEnd) params = params.append('rangeEnd', rangeEnd);
        return this._httpClient.get<HttpResponse>(route, {params}).pipe(
    ...
    getGroupAppliedRenewalsRate(groupId: string, rangeStart: string, rangeEnd: string): Observable<number> {
        const route: string = routes.groupAppliedRenewals(this._workspaceId, groupId);
        let params: HttpParams = new HttpParams();
        if(!!rangeStart) params = params.append('rangeStart', rangeStart);
        if(!!rangeEnd) params = params.append('rangeEnd', rangeEnd);
        return this._httpClient.get<HttpResponse>(route, {params}).pipe(
