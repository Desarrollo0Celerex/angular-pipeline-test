...
export class ChartPendingRenovationsService {
    ...
    getPendingRenovationsStats(
        ...
        requests.push(
            this._policyService.getPendingRenovationsStats(
                filters,
                rangeField,
    ...
    loadPendingRenovationsStatsData(
        pendingRenovationsStats: StatRangeData[][]
    ): void {
        const headerData: any[] = [
            ['Pólizas', 'Periodo Seleccionado', 'Periodo Comparación'],
        ];
        this.pendingRenovationsStatsData =
            ChartHelper.generateChartDataByRanges(
                pendingRenovationsStats,
                headerData
            );
    }
}
