...
export class CardGroupRenewalReportsComponent implements OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if(!!changes.groupId && changes.groupId.currentValue) {
            this.model.loadTotalRenewals(changes.groupId.currentValue, this.rangeStart, this.rangeEnd)
        }
    }
    get canDownloadReport(): boolean {
        ...
        switch(this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_RENEWALS:
                canDownloadReport = (this.model.totalGroupAppliedRenewals === 0) ? false : true;
            break;
            case this.model.REPORT_TYPES.PENDING_RENEWALS:
                canDownloadReport = (this.model.totalGroupPendingRenewals === 0) ? false : true;
            break;
        }
    ...
    get description(): string {
        ...
        switch(this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_RENEWALS:
                label = (this.model.totalGroupAppliedRenewals === 1) ? 'Renovación Aplicada' : 'Renovaciones Aplicadas';
                description = this.model.totalGroupAppliedRenewals + ' ' + label;
            break;
            case this.model.REPORT_TYPES.PENDING_RENEWALS:
                label = (this.model.totalGroupPendingRenewals === 1) ? 'Renovación Pendiente' : 'Renovaciones Pendientes';
                description = this.model.totalGroupPendingRenewals + ' ' + label;
            break;
        }
        return description;
    }
    ...
    get totalGroupRenewals(): number {
        return this.model.totalGroupAppliedRenewals + this.model.totalGroupPendingRenewals;
    }
