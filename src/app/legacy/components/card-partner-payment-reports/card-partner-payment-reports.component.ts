...
export class CardPartnerPaymentReportsComponent implements OnChanges {
    ...
    get canDownloadReport(): boolean {
        ...
        switch(this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_PAYMENTS:
                canDownloadReport = (this.model.totalPartnerAppliedPayments === 0) ? false : true;
            break;
            case this.model.REPORT_TYPES.PENDING_PAYMENTS:
                canDownloadReport = (this.model.totalPartnerPendingPayments === 0) ? false : true;
            break;
        }
    ...
    get description(): string {
        ...
        switch(this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_PAYMENTS:
                label = (this.model.totalPartnerAppliedPayments === 1) ? 'Recibo Aplicado' : 'Recibos Aplicados';
                description = this.model.totalPartnerAppliedPayments + ' ' + label;
            break;
            case this.model.REPORT_TYPES.PENDING_PAYMENTS:
                label = (this.model.totalPartnerPendingPayments === 1) ? 'Recibo Pendiente' : 'Recibos Pendientes';
                description = this.model.totalPartnerPendingPayments + ' ' + label;
            break;
        }
        return description;
    }
    ...
    get totalPartnerPayments(): number {
        return this.model.totalPartnerAppliedPayments + this.model.totalPartnerPendingPayments;
    }
