...
export class ContainerLeadConversionKpisService {
    kpis: KpiOne[] = [
        {
            ...
            description:
                'Permite identificar la efectividad en los cierres de venta.',
            totalContents: 0,
            selectedValue: 0,
            selectedRange: '',
            comparedValue: 0,
        ...
        },
        {
            ...
            subcontentName: 'Alto',
            description:
                'Permite identificar el porcentaje de conversión más alto.',
            totalContents: 0,
            selectedValue: 0,
            selectedRange: '',
            comparedValue: 0,
        ...
        },
        {
            ...
            subcontentName: 'Bajo',
            description:
                'Permite identificar el porcentaje de conversión más bajo.',
            totalContents: 0,
            selectedValue: 0,
            selectedRange: '',
            comparedValue: 0,
        ...
        },
        {
            ...
            description:
                'Permite identificar la debilidad en los cierres de venta.',
            totalContents: 0,
            selectedValue: 0,
            selectedRange: '',
            comparedValue: 0,
    ...
    getTotalWorkspaceGeneratedQuotes(
        range: ComparisonRangeData
    ): Observable<number[]> {
        return this._getWorkspaceQuotes(range, '');
    }

    getTotalWorkspaceAcceptedQuotes(
        range: ComparisonRangeData
    ): Observable<number[]> {
    ...
    getTotalWorkspaceRejectedQuotes(
        range: ComparisonRangeData
    ): Observable<number[]> {
        const filters: string = UtilitiesHelper.generateHttpFilter(
            'quotationStatusId',
    ...
    loadConversionRate(
        ...
        this.kpis[CONVERSION_RATE].selectedValue =
            totalQuotations[PERIOD_STATUS.SELECTED] > 0
                ? Math.round(
                      (acceptedQuotations[PERIOD_STATUS.SELECTED] * 100) /
                          totalQuotations[PERIOD_STATUS.SELECTED]
                  )
                : 0;
        this.kpis[CONVERSION_RATE].comparedValue =
            totalQuotations[PERIOD_STATUS.COMPARED] > 0
                ? Math.round(
                      (acceptedQuotations[PERIOD_STATUS.COMPARED] * 100) /
                          totalQuotations[PERIOD_STATUS.COMPARED]
                  )
                : 0;
    }
    ...
    loadHighestRate(
        totalQuotations: number[],
        sortedAcceptedQuotesByChannel: any[]
    ) {
        if (
            totalQuotations[PERIOD_STATUS.SELECTED] > 0 &&
            sortedAcceptedQuotesByChannel.length > 0
        ) {
            this.kpis[HEIGEST_RATE].selectedValue = Math.round(
                (sortedAcceptedQuotesByChannel[0][PERIOD_STATUS.SELECTED + 1] *
                    100) /
                    totalQuotations[PERIOD_STATUS.SELECTED]
            );
        } else {
        ...
        if (
            ...
            this.kpis[HEIGEST_RATE].comparedValue = Math.round(
                (sortedAcceptedQuotesByChannel[0][PERIOD_STATUS.COMPARED + 1] *
                    100) /
                    totalQuotations[PERIOD_STATUS.COMPARED]
            );
        } else {
    ...
    loadLowestRate(
        ...
        if (
            totalQuotations[PERIOD_STATUS.SELECTED] > 0 &&
            sortedAcceptedQuotesByChannel.length > 1
        ) {
            this.kpis[LOWEST_RATE].selectedValue = Math.round(
                (sortedAcceptedQuotesByChannel[
                    sortedAcceptedQuotesByChannel.length - 1
                ][PERIOD_STATUS.SELECTED + 1] *
                    100) /
                    totalQuotations[PERIOD_STATUS.SELECTED]
            );
        } else {
        ...
        if (
            ...
            this.kpis[LOWEST_RATE].comparedValue = Math.round(
                (sortedAcceptedQuotesByChannel[
                    sortedAcceptedQuotesByChannel.length - 1
                ][PERIOD_STATUS.COMPARED + 1] *
                    100) /
                    totalQuotations[PERIOD_STATUS.COMPARED]
            );
        } else {
    ...
    loadRejectionRate(
        ...
        this.kpis[REJECTION_RATE].selectedValue =
            totalQuotations[PERIOD_STATUS.SELECTED] > 0
                ? Math.round(
                      (rejectedQuotations[PERIOD_STATUS.SELECTED] * 100) /
                          totalQuotations[PERIOD_STATUS.SELECTED]
                  )
                : 0;
        this.kpis[REJECTION_RATE].comparedValue =
            totalQuotations[PERIOD_STATUS.COMPARED] > 0
                ? Math.round(
                      (rejectedQuotations[PERIOD_STATUS.COMPARED] * 100) /
                          totalQuotations[PERIOD_STATUS.COMPARED]
                  )
                : 0;
    }

    private _getWorkspaceQuotes(
        range: ComparisonRangeData,
        filters: string = ''
    ): Observable<number[]> {
        const rangeField: string = 'createdAt';
        ...
        requests.push(
            this._quotationService.getTotalWorkspaceQuotations(
                filters,
                rangeField,
                range.selectedRangeStart,
                range.selectedRangeEnd
