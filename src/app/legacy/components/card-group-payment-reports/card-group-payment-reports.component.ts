...
export class CardGroupPaymentReportsComponent implements OnChanges {
    ...
    get canDownloadReport(): boolean {
        ...
        switch (this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_PAYMENTS:
                canDownloadReport =
                    this.model.totalGroupAppliedPayments === 0 ? false : true;
                break;
            case this.model.REPORT_TYPES.PENDING_PAYMENTS:
                canDownloadReport =
                    this.model.totalGroupPendingPayments === 0 ? false : true;
                break;
        }
    ...
    get description(): string {
        ...
        switch (this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_PAYMENTS:
                label =
                    this.model.totalGroupAppliedPayments === 1
                        ? 'Recibo Aplicado'
                        : 'Recibos Aplicados';
                description =
                    this.model.totalGroupAppliedPayments + ' ' + label;
                break;
            case this.model.REPORT_TYPES.PENDING_PAYMENTS:
                label =
                    this.model.totalGroupPendingPayments === 1
                        ? 'Recibo Pendiente'
                        : 'Recibos Pendientes';
                description =
                    this.model.totalGroupPendingPayments + ' ' + label;
                break;
        }
        return description;
    }
    ...
    get totalGroupPayments(): number {
        return (
            this.model.totalGroupAppliedPayments +
            this.model.totalGroupPendingPayments
        );
    }
