...
export class ModalUpdateSinisterReportService {
    ...
    loadSinister(sinisterData: SinisterDataSend): Observable<Sinister> {
        const fields: string =
            'sinisterId,internalNumber,manager,sinisterNumber,invoice';
    ...
    updateSinister(sinisterData: SinisterDataSend): Observable<void> {
        const requestBody: UpdateSinisterReportDataSend =
            this.sinisterForm.value;
        return this._sinisterService.updatePolicySinisterReport(
            sinisterData,
            requestBody
        );
    }
