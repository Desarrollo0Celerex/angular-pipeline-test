...
export class modalConfirmApplyFractionalReceiptComponent implements OnChanges {
    ...
    @Output() actionConfirmed: EventEmitter<number> =
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (
            !!changes.fractionalReceiptAmount &&
            !!changes.fractionalReceiptAmount.currentValue
        ) {
    ...
    applyEndorsementWithFractionalReceipt(): void {
        ...
        if (this.model.form.valid) {
            ModalPlugin.hide(this.modalId);
            const fractionalReceiptAmount: number = parseFloat(
                UtilitiesHelper.removeCommasFromQuantity(
                    this.model.f.fractionalReceiptAmount.value
                )
            );
