...
export class ContainerInsurancesBySubcategoryService {
    ...
    getSubcategories(): Observable<HttpResponse> {
        ...
        return this._insuranceSubcategoryService.getInsuranceSubcategories(fields);
    }

    loadInsurances(insuranceSubcategories: InsuranceSubcategory[]): void {
        this.insurancesBySubcategories = [];
        this._getRequestToGetSubcategoryInsurances(insuranceSubcategories).subscribe( (res: HttpResponse[]) => {
            for(let index in insuranceSubcategories) {
                const insuranceSubcategory: InsuranceSubcategory = insuranceSubcategories[index];
    ...
    private _getRequestToGetSubcategoryInsurances(insuranceSubcategorys: InsuranceSubcategory[]): Observable<HttpResponse[]> {
        ...
        for(let insuranceSubcategory of insuranceSubcategorys) {
            let request: Observable<HttpResponse> = this._insuranceService.getSubcategoryInsurances(insuranceSubcategory.insuranceSubcategoryId, fields, sortBy);
