...
export class ChartPendingPaymentsService {
    ...
    getPendingPaymentsStats(
        ...
        requests.push(
            this._paymentService.getCollectionStats(
                filters,
                rangeField,
    ...
    loadPendingPaymentsStatsData(
        pendingPaymentsStats: StatRangeData[][]
    ): void {
        const headerData: any[] = [
            ['Cobranza', 'Periodo Seleccionado', 'Periodo Comparación'],
        ];
        this.pendingPaymentsStatsData = ChartHelper.generateChartDataByRanges(
