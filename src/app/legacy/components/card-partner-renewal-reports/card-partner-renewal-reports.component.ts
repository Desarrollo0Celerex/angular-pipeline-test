...
export class CardPartnerRenewalReportsComponent implements OnChanges {
    ...
    get canDownloadReport(): boolean {
        ...
        switch (this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_RENEWALS:
                canDownloadReport =
                    this.model.totalPartnerAppliedRenewals === 0 ? false : true;
                break;
            case this.model.REPORT_TYPES.PENDING_RENEWALS:
                canDownloadReport =
                    this.model.totalPartnerPendingRenewals === 0 ? false : true;
                break;
        }
    ...
    get description(): string {
        ...
        switch (this.model.selectedReportType) {
            case this.model.REPORT_TYPES.APPLIED_RENEWALS:
                label =
                    this.model.totalPartnerAppliedRenewals === 1
                        ? 'Renovación Aplicada'
                        : 'Renovaciones Aplicadas';
                description =
                    this.model.totalPartnerAppliedRenewals + ' ' + label;
                break;
            case this.model.REPORT_TYPES.PENDING_RENEWALS:
                label =
                    this.model.totalPartnerPendingRenewals === 1
                        ? 'Renovación Pendiente'
                        : 'Renovaciones Pendientes';
                description =
                    this.model.totalPartnerPendingRenewals + ' ' + label;
                break;
        }
        return description;
    }
    ...
    get totalPartnerRenewals(): number {
        return (
            this.model.totalPartnerAppliedRenewals +
            this.model.totalPartnerPendingRenewals
        );
    }
