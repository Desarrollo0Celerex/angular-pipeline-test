...
export class ModalUpdateReceiptPaidService {
    ...
    loadReceiptPaid(receiptPaidId: string): Observable<void> {
        const fields: string = 'policyNumber,paymentPlanName,validityStartDate,validityEndDate,receiptsAmount,applicationDate,currencyName,paymentTypeId,paymentReference';
    ...
    buildForm(): void {
        if(!!this.receiptPaid) {
            this.form = this._formBuilder.group({
                ...
                paymentTypeId: [this.receiptPaid.paymentTypeId, [Validators.required]],
                paymentReference: [this.receiptPaid.paymentReference, [Validators.required]],
                paymentEvidence: ['']
            });
    ...
    updateReceipPaid(contactId: string, policyId: string, paymentId: string, receiptPaidId: string): Observable<void> {
        const requestBody: FormData = this._getRequestBody();
        return this._receiptPaidService.updateReceiptPaid(contactId, policyId, paymentId, receiptPaidId, requestBody);
    }
