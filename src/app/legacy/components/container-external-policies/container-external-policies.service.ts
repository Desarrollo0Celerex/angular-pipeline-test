...
export class ContainerExternalPoliciesService {
    ...
    loadContactExternalPolicies(
        contactId: string,
        policyStatusId: number
    ): void {
        const externalPolicyStatusId: number[] =
            this._getExternalPolicyStatusId(policyStatusId);
        const fields: string =
    ...
    loadGroupExternalPolicies(groupId: string, policyStatusId: number): void {
        ...
        const fields: string =
            'externalPolicyId,isChecked,policyUrl,coveredProperty,validityStartDate,validityEndDate,policyAmount,policyNumber,insurerImageUrl,insuranceName,insuranceIcon,insuranceBackground,paymentMethodName,insuranceTypeName,currencyName,externalPolicyStatusId,externalPolicyStatusName,externalPolicyStatusDescription,lifeTime,contactId';
        this._externalPolicyService
            .getGroupExternalPolicies(groupId, fields, externalPolicyStatusId)
            .subscribe((res: HttpResponse) => {
                this.externalPolicies = res.data.items;
            });
    }

    loadPartnerExternalPolicies(
        partnerId: string,
        policyStatusId: number
    ): void {
        const externalPolicyStatusId: number[] =
    ...
    private _getExternalPolicyStatusId(policyStatusId: number): number[] {
        ...
        switch (policyStatusId) {
            ...
            default:
                externalPolicyStatusId = [
                    EXTERNAL_POLICY_STATUS.CURRENT,
                    EXTERNAL_POLICY_STATUS.INCOMPLETE,
                ];
        }
