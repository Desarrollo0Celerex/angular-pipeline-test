...
export class SelectWorkspaceInsurancesService {
    ...
    addWorkspaceInsurance(insuranceId: number): Observable<void> {
        return this._workspaceInsuranceService.addWorkspaceInsurance(
            insuranceId
        );
    }
    ...
    loadLicenseInsurances(): void {
        ...
        this._getRequestToGetLicensesInsurances().subscribe(
            (res: HttpResponse[]) => {
    ...
    loadWorkspaceInsurances(): Observable<void> {
        ...
        return this._workspaceInsuranceService
            .getWorkspaceInsurances(fields)
            .pipe(
                tap((res: Insurance[]) => {
    ...
    removeWorkspaceInsurance(insuranceId: number): Observable<void> {
        return this._workspaceInsuranceService.removeWorkspaceInsurance(
            insuranceId
        );
    }
    ...
    private _compareInsurances(): void {
        for (let indexLicense in this.licenseInsurances) {
            for (let indexInsurance in this.licenseInsurances[indexLicense]
                .insurances) {
                for (let workspaceInsurance of this.workspaceInsurances) {
                    if (
                        this.licenseInsurances[indexLicense].insurances[
                            indexInsurance
                        ].insuranceId === workspaceInsurance.insuranceId
                    ) {
                        this.licenseInsurances[indexLicense].insurances[
                            indexInsurance
                        ].hasActiveLeadGenerator = true;
                    } else if (
                        typeof this.licenseInsurances[indexLicense].insurances[
                            indexInsurance
                        ].hasActiveLeadGenerator == 'undefined'
                    ) {
                        this.licenseInsurances[indexLicense].insurances[
                            indexInsurance
    ...
    private _getRequestToGetLicensesInsurances(): Observable<HttpResponse[]> {
        ...
        for (let license of this.licenses) {
            let request: Observable<HttpResponse> =
                this._insuranceService.getLicenseInsurances(
                    license.licenseId,
                    fields,
                    sortBy
                );
