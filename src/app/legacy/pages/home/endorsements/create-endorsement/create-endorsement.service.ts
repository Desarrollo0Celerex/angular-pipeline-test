...
import { CONTACT_TYPES, EMAIL_LENGTH, ENDORSEMENT_TYPES, FREE_TEXT_LENGTH, INSURANCE_GROUPS, LONG_ALPHANUMERIC_LENGTH, MULTITEXT_LENGTH, SHORT_ALPHANUMERIC_LENGTH, TITULAR_NAME_LENGTH } from '@constants/global';
...
export class CreateEndorsementService {
    ...
    buildForm(): void {
        if(this.policy !== null) {
            this.form = this._formBuilder.group({
                ...
                endorsementValidityStartDate: ['', [Validators.required, ValidatorsHelper.date] ],
                endorsementValidityEndDate: ['', [Validators.required, ValidatorsHelper.date] ],
                ...
                titularEmail: [this.policy.titularEmail, [Validators.email, Validators.minLength(EMAIL_LENGTH.MIN), Validators.maxLength(EMAIL_LENGTH.MAX) ] ],
                ...
                validityEndDate: [moment(this.policy.validityEndDate).format('DD/MM/YYYY') || '', [Validators.required, ValidatorsHelper.date, ValidatorsHelper.dateGreaterThan(this.policy.validityEndDate)] ],
            });

            if(this.policy.contactTypeId === CONTACT_TYPES.PERSON) {
                this.form.addControl('titularGenderId', new FormControl(this.policy.titularGenderId || '', [Validators.required, ValidatorsHelper.number]));
                this.form.addControl('titularAge', new FormControl(this.policy.titularAge || '', [ValidatorsHelper.number]));
            }

            this.areSeveralInsured = PolicyInsuredHelper.checkAreSeveralInsured(this.policy.insuranceTypeId);
            if(this.areSeveralInsured === false && !!this.policy.insureds && this.policy.insureds.length > 0) {
    ...
    checkEndorsementAmounts(): boolean {
        ...
        let endorsementNetPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementNetPay.value));
        let endorsementFeePay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementFeePay.value));
        let endorsementCoverPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementCoverPay.value));
        let endorsementExtraPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementExtraPay.value));
        let endorsementDiscount: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementDiscount.value));
        let endorsementTaxPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementTaxPay.value));
        let endorsementTotalAmount: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementTotalAmount.value));
        endorsementNetPay = (endorsementNetPay < 0) ? endorsementNetPay * (-1) : endorsementNetPay;
        endorsementFeePay = (endorsementFeePay < 0) ? endorsementFeePay * (-1) : endorsementFeePay;
        endorsementCoverPay = (endorsementCoverPay < 0) ? endorsementCoverPay * (-1) : endorsementCoverPay;
        endorsementExtraPay = (endorsementExtraPay < 0) ? endorsementExtraPay * (-1) : endorsementExtraPay;
        endorsementDiscount = (endorsementDiscount < 0) ? endorsementDiscount * (-1) : endorsementDiscount;
        endorsementTaxPay = (endorsementTaxPay < 0) ? endorsementTaxPay * (-1) : endorsementTaxPay;
        endorsementTotalAmount = (endorsementTotalAmount < 0) ? endorsementTotalAmount * (-1) : endorsementTotalAmount;
        
        let endorsementTotalAmountAux: number = endorsementNetPay + endorsementTaxPay + endorsementFeePay + endorsementCoverPay + endorsementExtraPay - endorsementDiscount;
    ...
    createEndorsementWithCancellation(contactId: string, policyId: string): Observable<void> {
        const requestBody: FormData = this._getRequestBodyToCreateEndorsementWithCancellation();
        return this._policyService.createEndorsementWithCancellation(contactId, policyId, requestBody);
    }

    createEndorsementWithChanges(contactId: string, policyId: string): Observable<void> {
        const requestBody: FormData = this._getRequestBodyToCreateEndorsementWithChanges();
        return this._policyService.createEndorsementWithChanges(contactId, policyId, requestBody);
    }

    createEndorsementWithDecrement(contactId: string, policyId: string): Observable<void> {
        const requestBody: FormData = this._getRequestBodyToCreateEndorsementWithDecrement();
        return this._policyService.createEndorsementWithDecrement(contactId, policyId, requestBody);
    }

    createEndorsementWithIncrement(contactId: string, policyId: string, fractionalReceiptAmount: number, endorsementPaymentMethodId: number): Observable<void> {
        const requestBody: FormData = this._getRequestBodyToCreateEndorsementWithIncrement(fractionalReceiptAmount, endorsementPaymentMethodId);
    ...
    calculateFractionalReceiptAmount(endorsementTotalAmount: number): number {
        ...
        const endorsementValidityStartDate = moment(this.f.endorsementValidityStartDate.value, 'DD/MM/YYYY');
        const endorsementValidityEndDate = moment(this.f.endorsementValidityEndDate.value, 'DD/MM/YYYY');
        const policyValidityStartDate = moment(this.policy!.validityStartDate);
        const endorsementNetPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementNetPay.value));
        const endorsementCoverPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementCoverPay.value));
        const endorsementExtraPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementExtraPay.value));
        
        if(endorsementTotalAmount > 0 && endorsementValidityStartDate.isAfter(policyValidityStartDate)) {
            // Obtener los dias de vigencia del endoso (con la formula base360)
            const endorsementDays: number = UtilitiesHelper.days360(endorsementValidityStartDate.format('YYYY-MM-DD'), endorsementValidityEndDate.format('YYYY-MM-DD'));
            // Obtener el número de meses enteros que hay desde que inicia el endoso hasta que finaliza.
            const monthsDiff: number = endorsementValidityEndDate.diff(endorsementValidityStartDate, 'months');
            // calcular los días prorateados (días de vigencia del endoso - (meses enteros * 30) )
            const fractionalReceiptDays: number =  endorsementDays - (monthsDiff * 30);
            // calcular la prima diaria ((prima neta + cargados extra) / dias de vigencia del endoso)
            const dailyAmount: number = (endorsementNetPay + endorsementExtraPay)/ endorsementDays;
            // calcular la prima neta del pago fraccionado ( dias prorateados * prima diaria)
            const fractionalReceiptNetPay: number = fractionalReceiptDays * dailyAmount;
            // calcular el pago por financiamiento (prima neta * 0.09)
            const fractionalReceiptFeePay: number = fractionalReceiptNetPay * 0.09;
            // calcular el sub total del endoso (prima neta + pago por financiamiento + derechos de póliza)
            const fractionalReceiptSubTotal: number = fractionalReceiptNetPay + fractionalReceiptFeePay + endorsementCoverPay;
            // Calcular el iva del sub total (sub total * 0.16)
            const fractionalReceiptTaxPay: number = fractionalReceiptSubTotal * 0.16;
            // calcular el monto total del recibo fraccionado (prima neta + pago por financiamiento + derechos de póliza del endoso + iva)
            fractionalReceiptAmount = parseFloat((fractionalReceiptNetPay + fractionalReceiptFeePay + endorsementCoverPay + fractionalReceiptTaxPay).toFixed(2));
        }
    ...
    updatePolicyInsured(contactId: string, policyId: string): Observable<void> {
        const updateRequestBodies: FormData[] = this._getRequestBodiesToUpdatePolicyInsured();
        return this._policyInsuredService.updatePolicyInsureds(contactId, policyId, updateRequestBodies);
    }
    ...
    private _addEndorsementPaymentFields(): void {
        ...
        this.form.addControl('endorsementNetPay', new FormControl('0.00', [Validators.required, ValidatorsHelper.amount]));
        this.form.addControl('endorsementFeePay', new FormControl('0.00', [Validators.required, ValidatorsHelper.amount]));
        this.form.addControl('endorsementCoverPay', new FormControl('0.00', [Validators.required, ValidatorsHelper.amount]));
        this.form.addControl('endorsementExtraPay', new FormControl('0.00', [Validators.required, ValidatorsHelper.amount]));
        this.form.addControl('endorsementDiscount', new FormControl('0.00', [Validators.required, ValidatorsHelper.amount]));
        this.form.addControl('endorsementTaxPay', new FormControl('0.00', [Validators.required, ValidatorsHelper.amount]));
        this.form.addControl('endorsementTotalAmount', new FormControl('0.00', [Validators.required, ValidatorsHelper.amount]));
        this.form.addControl('paymentMethodId', new FormControl(this.policy!.paymentMethodId || '', [Validators.required]));
        this.form.addControl('paymentPlanId', new FormControl(this.policy!.paymentPlanId || '', [Validators.required]));
        this.form.addControl('bills', new FormControl(this.policy!.bills || 0, [Validators.required]));
        this.f.paymentPlanId.disable();
        this.f.bills.disable();
    }

    private _disableFormFields(): void {
        const fieldsToIgnore: string [] = ['endorsementFile', 'endorsementNumber', 'endorsementEmissionDate', 'endorsementValidityStartDate', 'endorsementValidityEndDate', 'endorsementTypeId', 'endorsementComments', 'evidenceFile'];
    ...
    private _getRequestBodyToCreateEndorsementWithCancellation(): FormData {
        ...
        requestBody.append('endorsementValidityStartDate', this.f.endorsementValidityStartDate.value);
        requestBody.append('endorsementValidityEndDate', this.f.endorsementValidityEndDate.value);
    ...
    private _getRequestBodyToCreateEndorsementWithDecrement(): FormData {
        ...
        requestBody.append('endorsementCoverPay', endorsementCoverPay.toString());
        requestBody.append('endorsementExtraPay', endorsementExtraPay.toString());
        requestBody.append('endorsementDiscount', endorsementDiscount.toString());
        requestBody.append('endorsementTaxPay', endorsementTaxPay.toString());
        requestBody.append('endorsementAmount', endorsementTotalAmount.toString());
    ...
    private _getRequestBodyToCreateEndorsementWithIncrement(fractionalReceiptAmount: number, endorsementPaymentMethodId: number): FormData {
        let endorsementNetPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementNetPay.value));
        let endorsementFeePay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementFeePay.value));
        let endorsementCoverPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementCoverPay.value));
        let endorsementExtraPay: number = parseFloat(UtilitiesHelper.removeCommasFromQuantity(this.f.endorsementExtraPay.value));
    ...
    private _getRequestBodiesToUpdatePolicyInsured(): FormData[] {
        ...
        for(let insured of insureds) {
            if(insured.status === 'VALID' && !!insured.value.policyInsuredId) {
                const requestBody: FormData = this._getRequestBodyToUpdatePolicyInsured(insured.value);
        ...
        switch (this.policy!.insuranceGroupId) {
            case INSURANCE_GROUPS.PEOPLE:
                insuredForm = this._formBuilder.group({
                    personName: [(!!insured.personName) ? insured.personName : '', [Validators.required, Validators.minLength(TITULAR_NAME_LENGTH.MIN), Validators.maxLength(TITULAR_NAME_LENGTH.MAX), ValidatorsHelper.ownName]],
                    personGenderId: [(!!insured.personGenderId) ? insured.personGenderId : ''],
                    personAge: [(!!insured.personAge) ? insured.personAge : '', [ValidatorsHelper.number]]
                });
                break;

            case INSURANCE_GROUPS.VEHICLES:
                insuredForm = this._formBuilder.group({
                    vehicleMaker: [(!!insured.vehicleMaker) ? insured.vehicleMaker : '', [Validators.required, Validators.minLength(LONG_ALPHANUMERIC_LENGTH.MIN), Validators.maxLength(50), ValidatorsHelper.alphanumeric]],
                    vehicleVersion: [(!!insured.vehicleVersion) ? insured.vehicleVersion : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(100), ValidatorsHelper.freeText]],
                    vehicleModel: [(!!insured.vehicleModel) ? insured.vehicleModel : '', [Validators.required, ValidatorsHelper.vehicleModel]],
                    vehiclePlates: [(!!insured.vehiclePlates) ? insured.vehiclePlates : '', [Validators.minLength(SHORT_ALPHANUMERIC_LENGTH.MIN), Validators.maxLength(SHORT_ALPHANUMERIC_LENGTH.MAX), ValidatorsHelper.alphanumeric]],
                    vehicleSerial: [(!!insured.vehicleSerial) ? insured.vehicleSerial : '', [Validators.minLength(LONG_ALPHANUMERIC_LENGTH.MIN), Validators.maxLength(100), ValidatorsHelper.alphanumeric]],
                    vehicleMotor: [(!!insured.vehicleMotor) ? insured.vehicleMotor : '', [Validators.minLength(LONG_ALPHANUMERIC_LENGTH.MIN), Validators.maxLength(100), ValidatorsHelper.alphanumeric]],
                });
                break;

            case INSURANCE_GROUPS.BUILDINGS:
                insuredForm = this._formBuilder.group({
                    buildingName: [(!!insured.buildingName) ? insured.buildingName : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(FREE_TEXT_LENGTH.MAX), ValidatorsHelper.freeText]],
                    buildingUsage: [(!!insured.buildingUsage) ? insured.buildingUsage : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(FREE_TEXT_LENGTH.MAX), ValidatorsHelper.freeText]],
                    buildingLocation: [(!!insured.buildingLocation) ? insured.buildingLocation : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(FREE_TEXT_LENGTH.MAX), ValidatorsHelper.freeText]]
                });
                break;
            ...
            case INSURANCE_GROUPS.RC:
                insuredForm = this._formBuilder.group({
                    objectName: [(!!insured.objectName) ? insured.objectName : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(FREE_TEXT_LENGTH.MAX), ValidatorsHelper.freeText]],
                    objectUsage: [(!!insured.objectUsage) ? insured.objectUsage : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(FREE_TEXT_LENGTH.MAX), ValidatorsHelper.freeText]],
                    objectDescription: [(!!insured.objectDescription) ? insured.objectDescription : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(FREE_TEXT_LENGTH.MAX), ValidatorsHelper.freeText]]
                });
                break;

            default:
                insuredForm = this._formBuilder.group({
                    policyDetails: [(!!insured.policyDetails) ? insured.policyDetails : '', [Validators.required, Validators.minLength(FREE_TEXT_LENGTH.MIN), Validators.maxLength(FREE_TEXT_LENGTH.MAX), ValidatorsHelper.freeText]]
                });
                break;
        }
