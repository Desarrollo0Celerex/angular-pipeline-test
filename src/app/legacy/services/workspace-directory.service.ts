...
export class WorkspaceDirectoryService {
    ...
    checkWorkspaceDirectoriesIsCompleted(
        filters: string = ''
    ): Observable<boolean> {
        const route: string = ROUTES.workspaceDirectoriesIsCompleted(
            this._workspaceId
        );
    ...
    saveWorkspaceDirectories(
        requestBody: SaveWorkspaceDirectoriesDataSend
    ): Observable<void> {
        const route: string = ROUTES.workspaceDirectories(this._workspaceId);
