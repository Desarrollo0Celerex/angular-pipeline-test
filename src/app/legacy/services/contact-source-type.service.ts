...
const ROUTES = {
    ...
    contactSourceTypesStats: (workspaceId: string, contactSourceId: number) =>  `${environment.apiUrl}/workspaces/${workspaceId}/stats/contact-sources/${contactSourceId}/contact-source-types`,
}
...
export class ContactSourceTypeService {
    ...
    getContactSourceTypesStats(contactSourceId: number, rangeField: string = '', rangeStart: string = '', rangeEnd: string = ''): Observable<Stat[]> {
        const route: string = ROUTES.contactSourceTypesStats(this._workspaceId, contactSourceId);
        let params: HttpParams = new HttpParams();
