...
import { CONTACT_TYPES, ENDORSEMENT_PAYMENT_METHODS, ENDORSEMENT_TYPES, FILE_ALL_FORMATS, FILE_TYPES, INSURANCE_GROUPS } from '@constants/global';
...
export class CreateEndorsementPage implements OnInit {
    ...
    calendarIdEndorsementValidityStartDate: string = 'endorsementValidityStartDate';
    ...
    modalIdConfirmApplyEndorsementWithChanges: string = 'agt-confirm-apply-endorsement-with-changes';
    ...
    modalIdEndorsementAmountsDifferent: string = 'agt-endorsement-amounts-different';
    ...
    get policyAmount(): number {
        return (this.model.policy !== null) ? parseFloat(this.model.policy.policyAmount.toString()) : 0;
    }
        ...
        switch (selectedEndorsementTypeId) {
            case ENDORSEMENT_TYPES.A:
                if(this.model.checkHasPolicyPendingReceipts()) {
                    ...
                    this.selectedEndorsementPaymentMethodId = ENDORSEMENT_PAYMENT_METHODS.SINGLE_RECEIPT;
    ...
    createEndorsementWithFractionalReceipt(fractionalReceiptAmount: number): void {
    ...
    selectEndorsementPaymentMethod(paymentMethodId: number): void {
        ...
        this.fractionalReceiptAmount = this.model.calculateFractionalReceiptAmount(this.endorsementTotalAmount);
    ...
    private _createEndorsementWithCancellation(): void {
        ...
        this.model.createEndorsementWithCancellation(this.contactId, this.policyId).subscribe( () => {
    ...
    private _createEndorsementWithChanges(): void {
        this._loadingService.show();
        this.model.createEndorsementWithChanges(this.contactId, this.policyId).subscribe( () => {
    ...
    private _createEndorsementWithDecrement(): void {
        this._loadingService.show();
        this.model.createEndorsementWithDecrement(this.contactId, this.policyId).subscribe( () => {
            this._handleSuccessfulEndorsementCreation();
        });
    }
    ...
    private _applyEndorsementWithIncrement(): void {
        this._loadingService.show();
        this.model.createEndorsementWithIncrement(this.contactId, this.policyId, this.fractionalReceiptAmount, this.selectedEndorsementPaymentMethodId).subscribe( () => {
            this._handleSuccessfulEndorsementCreation();
        });
    }
    ...
    private _goToPolicyHistory(): void {
        const url: string = ROUTES_NAME.showHistoryPolicy(this.contactId, this.policyId);
        this._router.navigateByUrl(url);
    }

    private _handleSuccessfulEndorsementCreation(): void {
        if(this.model.areSeveralInsured === false) {
            this.model.updatePolicyInsured(this.contactId, this.policyId).subscribe(() => {
    ...
    private _initCalendars(): void {
        ...
        DatePickerPlugin.initElement(this.calendarIdEndorsementValidityStartDate, this._onChangeDate, this);
        DatePickerPlugin.initElement(this.calendarIdEndorsementValidityEndDate, this._onChangeDate, this);
        DatePickerPlugin.initElement(this.calendarIdValidityEndDate, this._onChangeDate, this);
    }

    private _loadData(): void {
        this.model.getPolicy(this.contactId, this.policyId).subscribe((res: HttpResponse) => {
    ...
    private _loadPaymentPlans(): void {
        this.model.loadPaymentPlans().subscribe( () => {
            ...
            this.selectedPaymentPlanName = this.model.getPaymentPlanName(selectedPaymentPlanId);
            // PENDIENTE
            /*this.model.calculatePaymentPlansAvailable();
            this.model.calculateMonthsLeftToPay(); */
        });
    ...
    private _onChangeDate(selectorId: string, changedValue: string, context: CreateEndorsementPage): void {
