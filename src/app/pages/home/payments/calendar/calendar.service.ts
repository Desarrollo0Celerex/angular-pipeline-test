...
export class CalendarService {
    ...
    loadPayments(paymentDate: string, page: number): void {
        ...
        const filters: string = UtilitiesHelper.generateHttpFilter('paymentDate', [paymentDate]);
        this._paymentService.getPayments(page, fields, filters).subscribe((res: HttpResponse) => {
    ...
    loadTotalPaymentsAmount(selectedDate: string): void {
        ...
        this._paymentService.getTotalPaymentsAmount(rangeField, selectedDate, selectedDate).subscribe((res: TotalPaymentsAmountData) => {
    ...
    loadTotalPaymentsAmountByMonth(rangeStart: string, rangeEnd: string): void {
        const rangeField: string = 'paymentDate';
        this._paymentService.getTotalPaymentsAmount(rangeField, rangeStart, rangeEnd).subscribe((res: TotalPaymentsAmountData) => {
